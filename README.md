...
* SW9 is a small tactile button — the default/via keymaps use it to cycle layers; the extended keymap uses long-press to open a games/tools menu.

## Keymaps

Three keymaps live under `keymaps/`:

* **`default`** — minimal 4-layer keymap (media / editing / zoom / navigation). SW9 cycles layers with standard `TO()` keycodes. The basic keyboard-level OLED shows board name + active layer + the last-pressed key's label.
* **`via`** — same 4-layer layout with `VIA_ENABLE = yes`, so the layout (and the per-layer encoder map) is remappable from the VIA app at runtime. Same basic OLED.
* **`extended`** — the product firmware: VIA + on-board games (Snake, Dino) + a background Pomodoro timer (with a corner countdown and full-screen expiry alert) + a richer OLED UI (scrolling banner, big-font action labels, full-screen game / timer screens). SW9 long-press opens the games/tools menu.

## Compiling and Flashing

```
make sm_boards/macro2040:default      # add :flash to flash after build
make sm_boards/macro2040:via
make sm_boards/macro2040:extended
```
...
* **`keycode_label.c` / `.h`** — shared "keycode → short readable label" helper used by both the basic OLED above and the extended keymap's richer renderer.
