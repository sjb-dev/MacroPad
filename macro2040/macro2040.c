...
static void update_quadrature_pulse(void) {
    ...
    if (quad_direction) {
        switch (quad_state) {
            case QUAD_STATE_0: writePinHigh(AS5600_PIN_A); writePinHigh(AS5600_PIN_B); quad_next_time = now + (PULSE_DELAY / 1000); quad_state = QUAD_STATE_1; break;
            case QUAD_STATE_1: writePinLow(AS5600_PIN_A);  quad_next_time = now + (PHASE_DELAY / 1000); quad_state = QUAD_STATE_2; break;
            case QUAD_STATE_2: writePinLow(AS5600_PIN_B);  quad_next_time = now + (PHASE_DELAY / 1000); quad_state = QUAD_STATE_3; break;
            case QUAD_STATE_3: writePinHigh(AS5600_PIN_A); quad_next_time = now + (PHASE_DELAY / 1000); quad_state = QUAD_STATE_4; break;
            case QUAD_STATE_4: writePinHigh(AS5600_PIN_B); quad_state = QUAD_IDLE; break;
            default:           quad_state = QUAD_IDLE; break;
        }
    } else {
        switch (quad_state) {
            case QUAD_STATE_0: writePinHigh(AS5600_PIN_A); writePinHigh(AS5600_PIN_B); quad_next_time = now + (PULSE_DELAY / 1000); quad_state = QUAD_STATE_1; break;
            case QUAD_STATE_1: writePinLow(AS5600_PIN_B);  quad_next_time = now + (PHASE_DELAY / 1000); quad_state = QUAD_STATE_2; break;
            case QUAD_STATE_2: writePinLow(AS5600_PIN_A);  quad_next_time = now + (PHASE_DELAY / 1000); quad_state = QUAD_STATE_3; break;
            case QUAD_STATE_3: writePinHigh(AS5600_PIN_B); quad_next_time = now + (PHASE_DELAY / 1000); quad_state = QUAD_STATE_4; break;
            case QUAD_STATE_4: writePinHigh(AS5600_PIN_A); quad_state = QUAD_IDLE; break;
            default:           quad_state = QUAD_IDLE; break;
        }
    }
}
...
void keyboard_post_init_kb(void) {
    ...
    setPinOutput(AS5600_PIN_A);
    setPinOutput(AS5600_PIN_B);
    writePinHigh(AS5600_PIN_A);
    writePinHigh(AS5600_PIN_B);

    setPinInputHigh(GP22);
    setPinInputHigh(GP23);
